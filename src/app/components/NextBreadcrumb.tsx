...
export default function NextBreadcrumb({
  ...
  const segments = pathname.split("/").filter((seg) => seg && seg !== ""); // Ensure no empty or root segments
          ...
          {breadcrumbs.length === 0 ? null : (
            <li className="flex items-center gap-1">
              <div className="alllistitemsonnav">
                ...
                <Link
                  href="/about"
                  className="inline-flex items-center gap-1 px-3 py-1 rounded bg-white text-blue-700 hover:bg-blue-100 text-sm font-medium"
                >
                  About
                </Link>
                <Link
